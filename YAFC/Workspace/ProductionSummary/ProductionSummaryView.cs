...
namespace YAFC {
    public class ProductionSummaryView : ProjectPageView<ProductionSummary> {
        ...
        private class SummaryColumn : DataColumn<ProductionSummaryEntry> {
            ...
            public override void BuildElement(ImGui gui, ProductionSummaryEntry entry) {
                ...
                if (entry.subgroup != null) {
                    ...
                    ButtonEvent buttonEvent = gui.BuildButton(gui.lastRect, SchemeColor.None, SchemeColor.BackgroundAlt);
        ...
        private class GoodsColumn : DataColumn<ProductionSummaryEntry> {
            ...
            public override void BuildHeader(ImGui gui) {
                Rect moveHandle = gui.statePosition;
        ...
        private class RestGoodsColumn : TextDataColumn<ProductionSummaryEntry> {
            ...
            public override void BuildElement(ImGui gui, ProductionSummaryEntry data) {
                using ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(2.1f);
                foreach ((Goods goods, float amount) in data.flow) {
                    if (amount == 0f) {
                        continue;
                    }
                    ...
                    if (!view.model.columnsExist.Contains(goods)) {
                        grid.Next();
                        ButtonEvent evt = gui.BuildButton(goods.icon, amount > 0f ? SchemeColor.Green : SchemeColor.None, size: 1.5f);
        ...
        private void ApplyFilter(Goods goods) {
            Goods filter = filteredGoods == goods ? null : goods;
        ...
        private bool IsColumnsSynced() {
            ...
            int index = 2;
            foreach (ProductionSummaryColumn column in model.columns) {
        ...
        private void SyncGridHeaderWithColumns() {
            List<DataColumn<ProductionSummaryEntry>> columns = grid.columns;
            List<ProductionSummaryColumn> modelColumns = model.columns;
            columns.Clear();
            columns.Add(padding);
            columns.Add(firstColumn);
            foreach (ProductionSummaryColumn column in modelColumns) {
                if (!goodsToColumn.TryGetValue(column, out GoodsColumn currentColumn)) {
        ...
        private void AddOrRemoveColumn(Goods goods) {
            ...
            for (int i = 0; i < model.columns.Count; i++) {
                ProductionSummaryColumn column = model.columns[i];
        ...
        protected override void BuildContent(ImGui gui) {
            ...
            using (gui.EnterGroup(new Padding(1))) {
                ...
                using ImGuiUtils.InlineGridBuilder igrid = gui.EnterInlineGrid(3f, 1f);
                foreach ((Goods goods, float amount) in model.sortedFlow) {
                    igrid.Next();
