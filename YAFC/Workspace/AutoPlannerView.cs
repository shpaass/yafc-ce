...
namespace YAFC {
    public class AutoPlannerView : ProjectPageView<AutoPlanner> {
        ...
        protected override void BuildPageTooltip(ImGui gui, AutoPlanner contents) {
            ...
            foreach (AutoPlannerGoal goal in contents.goals) {
                grid.Next();
        ...
        private Action CreateAutoPlannerWizard(List<WizardPanel.PageBuilder> pages) {
            ...
            void Page1(ImGui gui, ref bool valid) {
                ...
                using (ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(3f)) {
                    for (int i = 0; i < goal.Count; i++) {
                        AutoPlannerGoal elem = goal[i];
                        grid.Next();
                        GoodsWithAmountEvent evt = gui.BuildFactorioObjectWithEditableAmount(elem.item, elem.amount, elem.item.flowUnitOfMeasure, out float newAmount);
            ...
            pages.Add(Page1);
            return () => {
                ProjectPage planner = MainScreen.Instance.AddProjectPage("Auto planner", goal[0].item, typeof(AutoPlanner), false, false);
        ...
        protected override void BuildContent(ImGui gui) {
            ...
            foreach (AutoPlannerRecipe[] tier in model.tiers) {
                using ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(3f);
                foreach (AutoPlannerRecipe recipe in tier) {
