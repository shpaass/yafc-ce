    ...
    public class ProductionTableView : ProjectPageView<ProductionTable> {
        ...
        private class RecipePadColumn : ProductionTableDataColumn {
            ...
            public override void BuildElement(ImGui gui, RecipeRow row) {
                ...
                if (row.parameters.warningFlags != 0) {
                    ...
                    if (hover) {
                        gui.ShowTooltip(g => {
                            ...
                            foreach ((WarningFlags flag, string text) in WarningsMeaning) {
                ...
                static void ToggleAll(bool state, ProductionTable table) {
                    foreach (RecipeRow recipe in table.recipes.Where(r => r.subgroup != null)) {
            ...
            private void BuildRowMarker(ImGui gui, RecipeRow row) {
                ...
                (Icon icon, SchemeColor color) = tagIcons[markerId];
        ...
        private class RecipeColumn : ProductionTableDataColumn {
            ...
            private void RemoveZeroRecipes(ProductionTable productionTable) {
                ...
                foreach (RecipeRow recipe in productionTable.recipes) {
            ...
            public override void BuildMenu(ImGui gui) {
                ...
                if (InputSystem.Instance.control && gui.BuildButton("Add ALL recipes") && gui.CloseDropdown()) {
                    foreach (Recipe recipe in Database.recipes.all) {
                        ...
                        RecipeRow row = view.AddRecipe(view.model, recipe);
goodsHaveNoProduction:;
            ...
            private void ExportIo(float multiplier) {
                ...
                foreach (ProductionLink link in view.model.links) {
                ...
                foreach (ProductionTableFlow flow in view.model.flow) {
        ...
        private class EntityColumn : ProductionTableDataColumn {
            ...
            public override void BuildElement(ImGui gui, RecipeRow recipe) {
                ...
                bool clicked;
                using (ImGui.Context group = gui.EnterGroup(default, RectAllocator.Stretch, spacing: 0f)) {
                    group.SetWidth(3f);
                    if (recipe.fixedBuildings > 0) {
                        GoodsWithAmountEvent evt = gui.BuildFactorioObjectWithEditableAmount(recipe.entity, recipe.fixedBuildings, UnitOfMeasure.None, out float newAmount);
            ...
            private void BuildSolarPanelAccumulatorView(ImGui gui, RecipeRow recipe) {
                EntityAccumulator accumulator = recipe.GetVariant(Database.allAccumulators);
            ...
            private void ShowEntityDropPown(ImGui imgui, RecipeRow recipe) {
                imgui.ShowDropDown(gui => {
                    ...
                    if (recipe.entity != null && gui.BuildButton("Create single building blueprint") && gui.CloseDropdown()) {
                        ...
                        (Item module, int count, bool beacon)[] modules = recipe.parameters.modules.modules;
                        if (modules != null) {
                            entity.items = new Dictionary<string, int>();
                            foreach ((Item module, int count, bool beacon) in recipe.parameters.modules.modules) {
                                if (!beacon) {
        ...
        private class IngredientsColumn : ProductionTableDataColumn {
            ...
            public override void BuildElement(ImGui gui, RecipeRow recipe) {
                ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(3f, 1f);
                if (recipe.isOverviewMode) {
                    ...
                    for (int i = 0; i < recipe.recipe.ingredients.Length; i++) {
                        ...
                        ProductionLink link = recipe.links.ingredients[i];
                        Goods goods = recipe.links.ingredientGoods[i];
                        grid.Next();
            ...
            public override void BuildElement(ImGui gui, RecipeRow recipe) {
                ...
                if (recipe.isOverviewMode) {
                    ...
                    for (int i = 0; i < recipe.recipe.products.Length; i++) {
                        Product product = recipe.recipe.products[i];
                        grid.Next();
        ...
        private class ModulesColumn : ProductionTableDataColumn {
            ...
            private void ModuleTemplateDrawer(ImGui gui, ProjectModuleTemplate element, int index) {
                ButtonEvent evt = gui.BuildContextMenuButton(element.name, icon: element.icon?.icon ?? default, disabled: !element.template.IsCompatibleWith(editingRecipeModules));
                if (evt == ButtonEvent.Click && gui.CloseDropdown()) {
                    ModuleTemplate copied = JsonUtils.Copy(element.template, editingRecipeModules, null);
            ...
            public override void BuildElement(ImGui gui, RecipeRow recipe) {
                ...
                using ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(3f);
            ...
            private void ShowModuleTemplateTooltip(ImGui gui, ModuleTemplate template) {
                gui.ShowTooltip(imGui => {
                    ...
                    using ImGuiUtils.InlineGridBuilder grid = imGui.EnterInlineGrid(3f, 1f);
            ...
            private void ShowModuleDropDown(ImGui gui, RecipeRow recipe) {
                Item[] modules = recipe.recipe.modules.Where(x => recipe.entity?.CanAcceptModule(x.module) ?? false).ToArray();
            ...
            public override void BuildMenu(ImGui gui) {
                ProductionTable model = view.model;
        ...
        private RecipeRow AddRecipe(ProductionTable table, Recipe recipe) {
            ...
            foreach (Ingredient ingr in recipeRow.recipe.ingredients) {
        ...
        private void CreateNewProductionTable(Goods goods, float amount) {
            ProjectPage page = MainScreen.Instance.AddProjectPage(goods.locName, goods, typeof(ProductionTable), true, false);
        ...
        private void OpenProductDropdown(ImGui targetGui, Rect rect, Goods goods, float amount, ProductionLink link, ProductDropdownType type, RecipeRow recipe, ProductionTable context, Goods[] variants = null) {
            ...
            DataUtils.FactorioObjectComparer<Recipe> comparer = DataUtils.GetRecipeComparerFor(goods);
            ...
            async void addRecipe(Recipe rec) {
                if (variants == null) {
                    ...
                    foreach (Goods variant in variants) {
            ...
            if (InputSystem.Instance.control) {
                ...
                Recipe[] recipeList = isInput ? goods.production : goods.usages;
                if (recipeList.SelectSingle(out Recipe selected)) {
                    addRecipe(selected);
                    return;
                }
            }
            ...
            Action<Goods> selectFuel = type != ProductDropdownType.Fuel ? null : (Action<Goods>)(fuel => {
                recipe.RecordUndo().fuel = fuel;
            });
            Recipe[] allProduction = goods == null ? Array.Empty<Recipe>() : variants == null ? goods.production : variants.SelectMany(x => x.production).Distinct().ToArray();
            ...
            void DropDownContent(ImGui gui) {
                ...
                if (type != ProductDropdownType.Product && goods != null && allProduction.Length > 0) {
                    ...
                    if (link == null) {
                        ...
                        ButtonEvent evt = gui.BuildButton(iconRect, SchemeColor.None, SchemeColor.Grey);
                        if (evt == ButtonEvent.Click && gui.CloseDropdown()) {
        ...
        private void DrawDesiredProduct(ImGui gui, ProductionLink element) {
            ...
            GoodsWithAmountEvent evt = gui.BuildFactorioObjectWithEditableAmount(element.goods, element.amount, element.goods.flowUnitOfMeasure, out float newAmount, error ? SchemeColor.Error : SchemeColor.Primary);
        ...
        private void BuildTableProducts(ImGui gui, ProductionTable table, ProductionTable context, ref ImGuiUtils.InlineGridBuilder grid) {
            ProductionTableFlow[] flow = table.flow;
        ...
        private void FillRecipeList(ProductionTable table, List<RecipeRow> list) {
            foreach (RecipeRow recipe in table.recipes) {
                list.Add(recipe);
                if (recipe.subgroup != null) {
        ...
        private void BuildShoppngList(RecipeRow recipeRoot) {
            ...
            List<RecipeRow> recipes = recipeRoot == null ? GetRecipesRecursive() : GetRecipesRecursive(recipeRoot);
            foreach (RecipeRow recipe in recipes) {
                if (recipe.entity != null) {
                    ...
                    if (recipe.parameters.modules.modules != null) {
                        foreach ((Item module, int count, bool beacon) module in recipe.parameters.modules.modules) {
                            if (!module.beacon) {
        ...
        private void BuildBeltInserterInfo(ImGui gui, float amount, float buildingCount) {
            ProjectPreferences prefs = Project.current.preferences;
            EntityBelt belt = prefs.defaultBelt;
            EntityInserter inserter = prefs.defaultInserter;
        ...
        private void BuildTableIngredients(ImGui gui, ProductionTable table, ProductionTable context, ref ImGuiUtils.InlineGridBuilder grid) {
            foreach (ProductionTableFlow flow in table.flow) {
                if (flow.amount >= 0f) {
                    break;
                }
        ...
        private void DrawRecipeTagSelect(ImGui gui, RecipeRow recipe) {
            using (gui.EnterRow()) {
                for (int i = 0; i < tagIcons.Length; i++) {
                    (Icon icon, SchemeColor color) = tagIcons[i];
                    ...
                    if (selected) {
                        ...
                        ButtonEvent evt = gui.BuildButton(gui.lastRect, SchemeColor.None, SchemeColor.BackgroundAlt, SchemeColor.BackgroundAlt);
                        if (evt) {
        ...
        protected override void BuildPageTooltip(ImGui gui, ProductionTable contents) {
            foreach (ProductionLink link in contents.links) {
                if (link.amount != 0f) {
                    using (gui.EnterRow()) {
            ...
            foreach (RecipeRow row in contents.recipes) {
        ...
        private void AddDesiredProductAtLevel(ProductionTable table) {
            SelectObjectPanel.Select(Database.goods.all, "Add desired product", product => {
                if (table.linkMap.TryGetValue(product, out ProductionLink existing)) {
        ...
        private void BuildSummary(ImGui gui, ProductionTable table) {
            ...
            using (gui.EnterGroup(pad)) {
                ...
                using ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(3f, 1f, elementsPerRow);
                foreach (ProductionLink link in table.links) {
                    if (link.amount != 0f) {
                        grid.Next();
            ...
            if (table.flow.Length > 0 && table.flow[0].amount < 0) {
                using (gui.EnterGroup(pad)) {
                    ...
                    ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(3f, 1f, elementsPerRow);
