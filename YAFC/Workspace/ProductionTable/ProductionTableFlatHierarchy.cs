...
namespace YAFC {
    public class FlatHierarchy<TRow, TGroup> where TRow : ModelObject<TGroup>, IGroupedElement<TGroup> where TGroup : ModelObject<ModelObject>, IElementGroup<TRow> {
        ...
        private void ActuallyMoveDraggingRecipe() {
            (TGroup parent, int index) = FindDragginRecipeParentAndIndex();
            if (parent == null) {
                return;
            }
        ...
        public void Build(ImGui gui) {
            ...
            SchemeColor bgColor = SchemeColor.PureBackground;
            int depth = 0;
            float depWidth = 0f;
            for (int i = 0; i < flatRecipes.Count; i++) {
                TRow recipe = flatRecipes[i];
                TGroup item = flatGroups[i];
                if (recipe != null) {
                    if (!recipe.visible) {
                        if (item != null) {
                    ...
                    if (buildExpandedGroupRows || item == null) {
                        Rect rect = grid.BuildRow(gui, recipe, depWidth);
            ...
            Rect fullRect = grid.EndBuildingContent(gui);
        ...
        private void BuildFlatHierarchy(TGroup table) {
            foreach (TRow recipe in table.elements) {
                flatRecipes.Add(recipe);
                TGroup sub = recipe.subgroup;
