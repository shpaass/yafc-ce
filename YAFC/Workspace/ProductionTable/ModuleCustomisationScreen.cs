    ...
    public class ModuleCustomisationScreen : PseudoScreen {
        ...
        public override void Build(ImGui gui) {
            ...
            if (template != null) {
                ...
                using ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(2f, 1f);
                for (int i = 0; i < template.filterEntities.Count; i++) {
                    Entity entity = template.filterEntities[i];
                    grid.Next();
            ...
            if (modules == null) {
                ...
                if (modules.beacon == null) {
                    ...
                    ModuleFillerParameters defaultFiller = recipe?.GetModuleFiller();
        ...
        private ICollection<Item> GetModules(EntityBeacon beacon) {
            Item[] modules = (beacon == null && recipe != null) ? recipe.recipe.modules : Database.allModules;
            EntityWithModules filter = ((EntityWithModules)beacon) ?? recipe?.entity;
            return filter == null ? modules : (ICollection<Item>)modules.Where(x => filter.CanAcceptModule(x.module)).ToArray();
        }
        ...
        private void DrawRecipeModules(ImGui gui, EntityBeacon beacon, ref ModuleEffects effects) {
            ...
            using ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(3f, 1f);
            List<RecipeRowCustomModule> list = beacon != null ? modules.beaconList : modules.list;
            foreach (RecipeRowCustomModule module in list) {
                grid.Next();
                GoodsWithAmountEvent evt = gui.BuildFactorioObjectWithEditableAmount(module.module, module.fixedCount, UnitOfMeasure.None, out float newAmount);
