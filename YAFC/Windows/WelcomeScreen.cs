...
namespace YAFC {
    public class WelcomeScreen : WindowUtility, IProgress<(string, string)> {
        ...
        private async void LoadProject() {
            try {
                (string dataPath, string modsPath, string projectPath, bool expensiveRecipes) = (this.dataPath, this.modsPath, path, expensive);
                ...
                Project project = FactorioDataSource.Parse(dataPath, modsPath, projectPath, expensiveRecipes, this, collector, Preferences.Instance.language);
                ...
                errorMessage = ex is LuaException lua ? lua.Message : ex.Message + "\n" + ex.StackTrace;
            }
            finally {
                loading = false;
                rootGui.Rebuild();
        ...
        private void BuildRecentProjectList(ImGui gui) {
            gui.spacing = 0f;
            foreach (ProjectDefinition project in Preferences.Instance.recentProjects) {
