    ...
    public class NeverEnoughItemsPanel : PseudoScreen, IComparer<NeverEnoughItemsPanel.RecipeEntry> {
        ...
        private readonly struct RecipeEntry {
            ...
            public RecipeEntry(Recipe recipe, bool isProduction, Goods currentItem, bool atCurrentMilestones) {
                ...
                if (!recipe.IsAccessible()) {
                    ...
                    entryStatus = recipe.specialType != FactorioObjectSpecialType.Normal && recipeFlow <= 0.01f
                        ? EntryStatus.Special
                        : waste > 0f ? EntryStatus.Normal : EntryStatus.Useful;
                }
            }
        }
        ...
        private void DrawProducts(ImGui gui, Recipe recipe) {
            ...
            for (int i = recipe.products.Length - 1; i >= 0; i--) {
                Product product = recipe.products[i];
        ...
        private void DrawTooManyThings(ImGui gui, IEnumerable<IFactorioObjectWrapper> list, int maxElemCount) {
            using ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(3f, 0f, maxElemCount);
            foreach (IFactorioObjectWrapper item in list) {
                grid.Next();
        ...
        private void DrawRecipeEntry(ImGui gui, RecipeEntry entry, bool production) {
            SchemeColor textcolor = SchemeColor.BackgroundText;
            SchemeColor bgColor = SchemeColor.Background;
            ...
            using (gui.EnterGroup(new Padding(0.5f), production ? RectAllocator.LeftRow : RectAllocator.RightRow, textcolor)) {
                ...
                gui.AllocateSpacing();
                RectAllocator textalloc = production ? RectAllocator.LeftAlign : RectAllocator.RightAlign;
                ...
                using (gui.EnterRow(0f, production ? RectAllocator.RightRow : RectAllocator.LeftRow)) {
                    ...
                    Rect iconRect = gui.AllocateRect(1f, 1f).Expand(0.25f);
        ...
        private void DrawEntryFooter(ImGui gui, bool production) {
            if (!production && current.fuelFor.Length > 0) {
                using (gui.EnterGroup(new Padding(0.5f), RectAllocator.LeftAlign)) {
                    ...
                    foreach (Entity fuelUsage in current.fuelFor) {
                        grid.Next();
        ...
        private void DrawEntryList(ImGui gui, List<RecipeEntry> entries, bool production) {
            ...
            EntryStatus prevEntryStatus = EntryStatus.Normal;
            FactorioObject prevLatestMilestone = null;
            foreach (RecipeEntry entry in entries) {
                EntryStatus status = entry.entryStatus;
                ...
                if (status == EntryStatus.NotAccessibleWithCurrentMilestones) {
                    FactorioObject latest = Milestones.Instance.GetHighest(entry.recipe, false);
                ...
                if (gui.ShouldBuildGroup(entry.recipe, out ImGui.BuildGroup group)) {
        ...
        public override void Build(ImGui gui) {
            ...
            using (ImGuiUtils.InlineGridBuilder split = gui.EnterHorizontalSplit(2)) {
                split.Next();
        ...
        int IComparer<RecipeEntry>.Compare(RecipeEntry x, RecipeEntry y) {
            ...
            if (x.entryStatus == EntryStatus.NotAccessibleWithCurrentMilestones) {
                Bits xMilestone = DataUtils.GetMilestoneOrder(x.recipe.id);
                Bits yMilestone = DataUtils.GetMilestoneOrder(y.recipe.id);
                if (xMilestone != yMilestone) {
                    return xMilestone.CompareTo(yMilestone);
                }
            }
            return x.flow != y.flow
                ? y.flow.CompareTo(x.flow)
                : x.recipe.RecipeWaste(atCurrentMilestones).CompareTo(y.recipe.RecipeWaste(atCurrentMilestones));
        }
    }
}
