...
namespace YAFC {
    public class DependencyExplorer : PseudoScreen {
        ...
        private void DrawFactorioObject(ImGui gui, FactorioId id) {
            FactorioObject fobj = Database.objects[id];
        ...
        private void DrawDependencies(ImGui gui) {
            gui.spacing = 0f;
            foreach (DependencyList data in Dependencies.dependencyList[current]) {
                if (!dependencyListTexts.TryGetValue(data.flags, out (string name, string missingText) dependencyType)) {
                ...
                if (data.elements.Length > 0) {
                    ...
                    gui.AllocateSpacing(0.5f);
                    foreach (FactorioId id in data.elements.OrderByDescending(x => CostAnalysis.Instance.flow[x])) {
        ...
        private void DrawDependants(ImGui gui) {
            gui.spacing = 0f;
            foreach (FactorioId reverseDependency in Dependencies.reverseDependencies[current].OrderByDescending(x => CostAnalysis.Instance.flow[x])) {
