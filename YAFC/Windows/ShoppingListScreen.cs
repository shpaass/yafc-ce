...
namespace YAFC {
    public class ShoppingListScreen : PseudoScreen {
        ...
        public static void Show(Dictionary<FactorioObject, int> counts) {
            ...
            foreach ((FactorioObject obj, float count) in Instance.list.data) {
                if (obj is Entity) {
                    buildings += count;
                }
        ...
        private List<(T, int)> ExportGoods<T>() where T : Goods {
            ...
            foreach ((FactorioObject element, float amount) in list.data) {
        ...
        private void ExportBlueprintDropdown(ImGui gui) {
            ...
            if (Database.objectsByTypeName.TryGetValue("Entity.constant-combinator", out FactorioObject combinator) && gui.BuildFactorioObjectButtonWithText(combinator) && gui.CloseDropdown()) {
                _ = BlueprintUtilities.ExportConstantCombinators("Shopping list", ExportGoods<Goods>());
            }

            foreach (EntityContainer container in Database.allContainers) {
        ...
        private Recipe FindSingleProduction(Recipe[] prodiuction) {
            Recipe current = null;
            foreach (Recipe recipe in prodiuction) {
        ...
        private void Decompose() {
            ...
            foreach ((FactorioObject item, float count) in list.data) {
                AddDecomposition(item, count);
            }

            int steps = 0;
            while (decompositionQueue.Count > 0) {
                FactorioObject elem = decompositionQueue.Dequeue();
