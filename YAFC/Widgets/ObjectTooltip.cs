    ...
    public class ObjectTooltip : Tooltip {
        ...
        private void BuildHeader(ImGui gui) {
            using (gui.EnterGroup(new Padding(1f, 0.5f), RectAllocator.LeftAlign, spacing: 0f)) {
                ...
                Bits milestoneMask = Milestones.Instance.GetMilestoneResult(target.target);
                if (milestoneMask.HighestBitSet() > 0) {
                    ...
                    using (gui.EnterRow(spacing)) {
                        int maskBit = 1;
                        foreach (FactorioObject milestone in Milestones.Instance.currentMilestones) {
            ...
            if (entity.energy != null) {
                ...
                using (gui.EnterGroup(contentPadding)) {
                    ...
                    if (entity.energy.emissions != 0f) {
                        SchemeColor emissionColor = SchemeColor.BackgroundText;
                        if (entity.energy.emissions < 0f) {
        ...
        private void BuildRecipe(RecipeOrTechnology recipe, ImGui gui) {
            ...
            using (gui.EnterGroup(contentPadding)) {
                ...
                if (recipe is Recipe rec) {
                    ...
                    if (waste > 0.01f) {
                        ...
                        SchemeColor color = wasteAmount < 90 ? SchemeColor.BackgroundText : SchemeColor.Error;
            ...
            if (recipe.modules.Length > 0) {
                ...
                AllowedEffects crafterCommonModules = AllowedEffects.All;
            ...
            if (recipe is Recipe lockedRecipe && !lockedRecipe.enabled) {
                ...
                using (gui.EnterGroup(contentPadding)) {
                    if (lockedRecipe.technologyUnlock.Length > 2) {
                        ...
                        foreach (Technology technology in lockedRecipe.technologyUnlock) {
                            Ingredient ingredient = TechnologyScienceAnalysis.Instance.GetMaxTechnologyIngredient(technology);
        ...
        private void BuildTechnology(Technology technology, ImGui gui) {
            ...
            Ingredient[] packs = TechnologyScienceAnalysis.Instance.allSciencePacks[technology];
            if (packs.Length > 0) {
                BuildSubHeader(gui, "Total science required");
                using (gui.EnterGroup(contentPadding)) {
                    using ImGuiUtils.InlineGridBuilder grid = gui.EnterInlineGrid(3f);
                    foreach (Ingredient pack in packs) {
                        grid.Next();
