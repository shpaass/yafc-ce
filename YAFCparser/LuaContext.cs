...
namespace YAFC.Parser {
    ...
    internal class LuaContext : IDisposable {
        ...
        public LuaContext() {
            ...
            LuaTable mods = NewTable();
            foreach (KeyValuePair<string, FactorioDataSource.ModInfo> mod in FactorioDataSource.allMods) {
        ...
        private int Require(IntPtr lua) {
            ...
            (string mod, string source) = fullChunkNames[traceId];
        ...
        private byte[] GetData(int index) {
            IntPtr ptr = lua_tolstring(L, index, out IntPtr len);
        ...
        public int Exec(ReadOnlySpan<byte> chunk, string mod, string name, int argument = 0) {
            ...
            Result result = luaL_loadbufferx(L, in chunk.GetPinnableReference(), (IntPtr)chunk.Length, name, null);
