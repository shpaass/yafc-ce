...
namespace YAFC.Parser {
    public static class FactorioDataSource {
        ...
        public static byte[] ReadModFile(string modName, string path) {
            ModInfo info = allMods[modName];
            if (info.zipArchive != null) {
                ZipArchiveEntry entry = info.zipArchive.GetEntry(info.folder + path);
                if (entry == null) {
                    return null;
                }
        ...
        private static void FindMods(string directory, IProgress<(string, string)> progress, List<ModInfo> mods) {
            ...
            foreach (string fileName in Directory.EnumerateFiles(directory)) {
                if (fileName.EndsWith(".zip", StringComparison.OrdinalIgnoreCase)) {
                    ...
                    ZipArchiveEntry infoEntry = zipArchive.Entries.FirstOrDefault(x =>
        ...
        public static Project Parse(string factorioPath, string modPath, string projectPath, bool expensive, IProgress<(string, string)> progress, ErrorCollector errorCollector, string locale, bool renderIcons = true) {
            ...
            try {
                ...
                if (File.Exists(modListPath)) {
                    ModList mods = JsonSerializer.Deserialize<ModList>(File.ReadAllText(modListPath));
                ...
                foreach (ModInfo mod in allFoundMods) {
                ...
                foreach ((string name, ModInfo mod) in allMods) {
                ...
                do {
                    ...
                    foreach (string mod in modsToDisable) {
                        _ = allMods.Remove(mod, out ModInfo disabled);
                        disabled?.Dispose();
                    }
                ...
                Project project = deserializer.LoadData(projectPath, dataContext.data, dataContext.defines["prototypes"] as LuaTable, progress, errorCollector, renderIcons);
                ...
                dataContext?.Dispose();
                foreach (KeyValuePair<string, ModInfo> mod in allMods) {
                    mod.Value?.Dispose();
                }
        ...
        internal class ModInfo : IDisposable {
            ...
            public static ModInfo FromJson(ReadOnlySpan<byte> json) {
                ModInfo info = JsonSerializer.Deserialize<ModInfo>(json.CleanupBom());
                _ = Version.TryParse(info.version, out Version parsedV);
            ...
            public void ParseDependencies() {
                ...
                foreach (string dependency in dependencies) {
                    Match match = dependencyRegex.Match(dependency);
                    if (match.Success) {
            ...
            public bool CheckDependencies(Dictionary<string, ModInfo> allMods, List<string> modsToDisable) {
                foreach ((string mod, bool optional) in parsedDependencies) {
            ...
            public bool CanLoad(Dictionary<string, ModInfo> mods, HashSet<string> nonLoadedMods) {
                foreach ((string mod, _) in parsedDependencies) {
        ...
        public static IEnumerable<string> GetAllModFiles(string mod, string prefix) {
            ModInfo info = allMods[mod];
            if (info.zipArchive != null) {
                prefix = info.folder + prefix;
                foreach (ZipArchiveEntry entry in info.zipArchive.Entries) {
