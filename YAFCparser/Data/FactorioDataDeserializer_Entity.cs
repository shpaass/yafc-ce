...
namespace YAFC.Parser {
    internal partial class FactorioDataDeserializer {
        ...
        private int CountFluidBoxes(LuaTable list, bool input) {
            int count = 0;
            foreach (LuaTable fluidBox in list.ArrayElements<LuaTable>()) {
        ...
        private void ReadFluidEnergySource(LuaTable energySource, Entity entity) {
            ...
            if (GetFluidBoxFilter(energySource, "fluid_box", 0, out Fluid fluid, out TemperatureRange filterTemperature)) {
        ...
        private Recipe CreateLaunchRecipe(EntityCrafter entity, Recipe recipe, int partsRequired, int outputCount) {
            ...
            Recipe launchRecipe = CreateSpecialRecipe(recipe, launchCategory, "launch");
        ...
        private void DeserializeRocketEntities(LuaTable data) {
            ...
            foreach (System.Collections.Generic.KeyValuePair<object, object> entry in data.ObjectElements) {
        ...
        private void DeserializeEntity(LuaTable table) {
            ...
            switch (factorioType) {
                case "transport-belt":
                    ...
                    break;
                case "inserter":
                    EntityInserter inserter = GetObject<Entity, EntityInserter>(name);
                    inserter.inserterSwingTime = 1f / (table.Get("rotation_speed", 1f) * 60);
                    inserter.isStackInserter = table.Get("stack", false);
                    break;
                case "accumulator":
                    EntityAccumulator accumulator = GetObject<Entity, EntityAccumulator>(name);
                    ...
                    break;
                case "reactor":
                    EntityReactor reactor = GetObject<Entity, EntityReactor>(name);
                    ...
                    break;
                case "beacon":
                    EntityBeacon beacon = GetObject<Entity, EntityBeacon>(name);
                ...
                case "container":
                    EntityContainer container = GetObject<Entity, EntityContainer>(name);
                    ...
                    break;
                case "character":
                    EntityCrafter character = GetObject<Entity, EntityCrafter>(name);
                    ...
                    break;
                case "boiler":
                    EntityCrafter boiler = GetObject<Entity, EntityCrafter>(name);
                    ...
                    _ = GetFluidBoxFilter(table, "fluid_box", 0, out Fluid input, out TemperatureRange acceptTemperature);
                    _ = table.Get("target_temperature", out int targetTemp);
                    Fluid output = hasOutput ? GetFluidBoxFilter(table, "output_fluid_box", targetTemp, out Fluid fluid, out _) ? fluid : null : input;
                    if (input == null || output == null) // TODO - boiler works with any fluid - not supported
{
                    ...
                    Recipe recipe = CreateSpecialRecipe(output, category, "boiling to " + targetTemp + "°");
                ...
                case "rocket-silo":
                case "furnace":
                    EntityCrafter crafter = GetObject<Entity, EntityCrafter>(name);
                    _ = table.Get("energy_usage", out usesPower);
                    ...
                    if (factorioType == "rocket-silo") {
                        ...
                        if (resultInventorySize > 0) {
                            ...
                            if (fixedRecipe != null) {
                                Recipe launchRecipe = CreateLaunchRecipe(crafter, fixedRecipe, partsRequired, outputCount);
                                ...
                                foreach (string categoryName in recipeCrafters.GetRaw(crafter).ToArray()) {
                                    foreach (RecipeOrTechnology possibleRecipe in recipeCategories.GetRaw(categoryName)) {
                ...
                case "burner-generator":
                    EntityCrafter generator = GetObject<Entity, EntityCrafter>(name);
                    ...
                    break;
                case "mining-drill":
                    EntityCrafter drill = GetObject<Entity, EntityCrafter>(name);
                    _ = table.Get("energy_usage", out usesPower);
                    ...
                    break;
                case "offshore-pump":
                    EntityCrafter pump = GetObject<Entity, EntityCrafter>(name);
                    ...
                    break;
                case "lab":
                    EntityCrafter lab = GetObject<Entity, EntityCrafter>(name);
                    _ = table.Get("energy_usage", out usesPower);
                    ...
                    break;
                case "solar-panel":
                    EntityCrafter solarPanel = GetObject<Entity, EntityCrafter>(name);
                ...
                case "electric-energy-interface":
                    EntityCrafter eei = GetObject<Entity, EntityCrafter>(name);
            ...
            if (table.Get("minable", out LuaTable minable)) {
                Product[] products = LoadProductList(minable);
                if (factorioType == "resource") {
                    // mining resource is processed as a recipe
                    _ = table.Get("category", out string category, "basic-solid");
                    Recipe recipe = CreateSpecialRecipe(entity, SpecialNames.MiningRecipe + category, "mining");
        ...
        private float EstimateNoiseExpression(LuaTable expression) {
            ...
            switch (type) {
                ...
                case "function-application":
                    ...
                    LuaTable args = expression.Get<LuaTable>("arguments", null);
                    switch (funName) {
                        case "add":
                            float res = 0f;
                            foreach (LuaTable el in args.ArrayElements<LuaTable>()) {
                        ...
                        case "spot-noise":
                            ...
                            float spotCount;
                            spotCount = args.Get("candidate_spot_count", out LuaTable spots)
                                ? EstimateNoiseExpression(spots)
                                : EstimateArgument(args, "candidate_point_count", 256) / EstimateArgument(args, "skip_span", 1);
