...
namespace YAFC.Parser {
    internal partial class FactorioDataDeserializer {
        ...
        private T GetRef<T>(LuaTable table, string key) where T : FactorioObject, new() {
            _ = GetRef<T>(table, key, out T result);
            return result;
        }
        ...
        private Fluid GetFluidFixedTemp(string key, int temperature) {
            ...
            if (registeredObjects.TryGetValue((typeof(Fluid), idWithTemp), out FactorioObject fluidWithTemp)) {
        ...
        private void UpdateSplitFluids() {
            ...
            foreach (Fluid fluid in allObjects.OfType<Fluid>()) {
        ...
        public Project LoadData(string projectPath, LuaTable data, LuaTable prototypes, IProgress<(string, string)> progress, ErrorCollector errorCollector, bool renderIcons) {
            ...
            UpdateSplitFluids();
            Task iconRenderTask = renderIcons ? Task.Run(RenderIcons) : Task.CompletedTask;
            ...
            try {
                ...
                foreach (FactorioObject o in allObjects) {
                    ...
                    if (o.iconSpec != null && o.iconSpec.Length > 0) {
                        ...
                        if (simpleSprite && simpleSpritesCache.TryGetValue(o.iconSpec[0].path, out Icon icon)) {
                            o.icon = icon;
                            continue;
                        }
                ...
                foreach (((string mod, string path) _, IntPtr image) in cache) {
        ...
        private unsafe Icon CreateIconFromSpec(Dictionary<(string mod, string path), IntPtr> cache, params FactorioIconPart[] spec) {
            ...
            IntPtr targetSurface = SDL.SDL_CreateRGBSurfaceWithFormat(0, IconSize, IconSize, 0, SDL.SDL_PIXELFORMAT_RGBA8888);
            _ = SDL.SDL_SetSurfaceBlendMode(targetSurface, SDL.SDL_BlendMode.SDL_BLENDMODE_BLEND);
            foreach (FactorioIconPart icon in spec) {
                (string mod, string path) modpath = FactorioDataSource.ResolveModPath("", icon.path);
                if (!cache.TryGetValue(modpath, out IntPtr image)) {
                    ...
                    if (imageSource == null) {
                        ...
                        fixed (byte* data = imageSource) {
                            IntPtr src = SDL.SDL_RWFromMem((IntPtr)data, imageSource.Length);
                            image = SDL_image.IMG_Load_RW(src, (int)SDL.SDL_bool.SDL_TRUE);
                            if (image != IntPtr.Zero) {
                                ref SDL.SDL_Surface surface = ref RenderingUtils.AsSdlSurface(image);
                ...
                ref SDL.SDL_Surface sdlSurface = ref RenderingUtils.AsSdlSurface(image);
        ...
        private void DeserializePrototypes(LuaTable data, string type, Action<LuaTable> deserializer, IProgress<(string, string)> progress) {
            ...
            foreach (KeyValuePair<object, object> entry in luaTable.ObjectElements) {
        ...
        private void DeserializeItem(LuaTable table) {
            ...
            if (item.factorioType == "module" && table.Get("effect", out LuaTable moduleEffect)) {
                ...
                if (table.Get("limitation", out LuaTable limitation)) {
                    Recipe[] limitationArr = limitation.ArrayElements<string>().Select(GetObject<Recipe>).ToArray();
                    if (limitationArr.Length > 0) {
                        item.module.limitation = limitationArr;
                        foreach (Recipe recipe in item.module.limitation) {
        ...
        private Goods LoadItemOrFluid(LuaTable table, bool useTemperature, string nameField = "name") {
            return !table.Get(nameField, out string name)
                ? null
                : table.Get("type", out string type) && type == "fluid"
                ? useTemperature
                    ? GetFluidFixedTemp(name, table.Get("temperature", out int temperature) ? temperature : 0)
                    : (Goods)GetObject<Fluid>(name)
                : GetObject<Item>(name);
        }
        ...
        private void Localize(string key, LuaTable table) {
            ...
            using IEnumerator<string> parts = ((IEnumerable<string>)key.Split("__")).GetEnumerator();
