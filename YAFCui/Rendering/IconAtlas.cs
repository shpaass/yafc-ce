...
namespace YAFC.UI {
    public class IconAtlas {
        ...
        public void DrawIcon(IntPtr renderer, Icon icon, SDL.SDL_Rect position, SDL.SDL_Color color) {
            ...
            prevRender = renderer;
            int index = (int)icon;
            ref TextureInfo texture = ref textures[0];
            ...
            if (!texture.existMap[index]) {
                IntPtr iconSurfacePtr = IconCollection.GetIconSurface(icon);
                if (iconSurfacePtr == IntPtr.Zero) {
                    Console.Error.WriteLine("Non-existing icon: " + icon);
                    return;
                }
                ref SDL.SDL_Surface iconSurface = ref RenderingUtils.AsSdlSurface(iconSurfacePtr);
