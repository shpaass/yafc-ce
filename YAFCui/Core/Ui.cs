    ...
    public static class Ui {
        ...
        public static void ProcessEvents() {
            try {
                ...
                foreach ((uint _, Window window) in windows) {
                ...
                bool hasEvents = (delta <= 0 ? SDL.SDL_PollEvent(out SDL.SDL_Event evt) : SDL.SDL_WaitEventTimeout(out evt, (int)delta)) != 0;
                ...
                while (hasEvents) {
                    switch (evt.type) {
                        case SDL.SDL_EventType.SDL_QUIT:
                            if (!quit) {
                                quit = true;
                                foreach ((uint _, Window v) in windows) {
                                    if (v.preventQuit) {
                                        quit = false;
                                        break;
                                    }
                        ...
                        case SDL.SDL_EventType.SDL_WINDOWEVENT:
                            if (!windows.TryGetValue(evt.window.windowID, out Window window)) {
                                break;
                            }
