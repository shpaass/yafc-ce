...
namespace YAFC.UI {
    public abstract class Scrollable : IKeyboardFocus {
        ...
        public void Build(ImGui gui, float height, bool useBottomPadding = false) {
            ...
            Vector2 scrollSize = size * size / (size + maxScroll);
            scrollSize = Vector2.Max(scrollSize, Vector2.One);
            Vector2 scrollStart = _scroll / maxScroll * (size - scrollSize);
    ...
    public class VirtualScrollList<TData> : ScrollAreaBase {
        ...
        protected override void BuildContents(ImGui gui) {
            ...
            using ImGui.Context manualPlacing = gui.EnterFixedPositioning(gui.width, rowCount * elementSize.Y, default);
            Vector2 offset = gui.statePosition.Position;
