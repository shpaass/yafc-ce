...
namespace YAFC.Model {
    public static class Database {
        ...
        public static FactorioObject FindClosestVariant(string id) {
            ...
            if (objectsByTypeName.TryGetValue(baseId, out FactorioObject result)) {
                return result;
            }

            if (fluidVariants.TryGetValue(baseId, out List<Fluid> variants)) {
                Fluid prev = variants[0];
                for (int i = 1; i < variants.Count; i++) {
                    Fluid cur = variants[i];
    ...
    public class FactorioIdRange<T> where T : FactorioObject {
        ...
        public Mapping<T, TValue> CreateMapping<TValue>(Func<T, TValue> mapFunc) {
            Mapping<T, TValue> map = CreateMapping<TValue>();
            foreach (T o in all) {
                map[o] = mapFunc(o);
            }
    ...
    public readonly struct Mapping<TKey, TValue> : IDictionary<TKey, TValue> where TKey : FactorioObject {
        ...
        public Mapping<TKey, TOther> Remap<TOther>(Func<TKey, TValue, TOther> remap) {
            Mapping<TKey, TOther> remapped = source.CreateMapping<TOther>();
            foreach (TKey key in source.all) {
