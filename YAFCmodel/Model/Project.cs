...
namespace YAFC.Model {
    public class Project : ModelObject {
        ...
        private void UpdatePageMapping() {
            ...
            foreach (Guid page in displayPages) {
                if (pagesByGuid.TryGetValue(page, out ProjectPage dpage)) {
                    dpage.visible = true;
                }
            }
        ...
        public ProjectPage FindPage(Guid guid) {
            ...
            return pagesByGuid.TryGetValue(guid, out ProjectPage page) ? page : null;
        }
    ...
    public class ProjectSettings : ModelObject<Project> {
        ...
        public void SetFlag(FactorioObject obj, ProjectPerItemFlags flag, bool set) {
            _ = itemFlags.TryGetValue(obj, out ProjectPerItemFlags flags);
            ProjectPerItemFlags newFlags = set ? flags | flag : flags & ~flag;
            if (newFlags != flags) {
                _ = this.RecordUndo();
        ...
        public ProjectPerItemFlags Flags(FactorioObject obj) {
            return itemFlags.TryGetValue(obj, out ProjectPerItemFlags val) ? val : 0;
        }
    ...
    public class ProjectPreferences : ModelObject<Project> {
        ...
        public (float multiplier, string suffix) GetItemPerTimeUnit() {
            return itemUnit == 0f ? GetPerTimeUnit() : ((float multiplier, string suffix))(1f / itemUnit, "b");
        }

        public (float multiplier, string suffix) GetFluidPerTimeUnit() {
            return fluidUnit == 0f ? GetPerTimeUnit() : ((float multiplier, string suffix))(1f / fluidUnit, "p");
        }

        public void SetSourceResource(Goods goods, bool value) {
            _ = this.RecordUndo();
            _ = value ? sourceResources.Add(goods) : sourceResources.Remove(goods);
        }
        ...
        public void ToggleFavourite(FactorioObject obj) {
            ...
            _ = favourites.Contains(obj) ? favourites.Remove(obj) : favourites.Add(obj);
        }
    }
